...
class ExcessoDeCaracteresError(Exception):
    pass
...
def esteganografar(imagem:str, texto:str, end=";"):
    ...
    try:
      ...
      qtdCaracteres = int((len(img1) * len(img1[0]) * 3) / 4)

      if qtdCaracteres < len(texto):
        raise ExcessoDeCaracteresError("O arquivo selecionado não suporta tantos caracteres, selecione outro arquivo ou diminua o texto")
    ...
    except ExcessoDeCaracteresError as err:
      return "",str(err)
